...
\mu_i & = \beta_0 + \beta_1 \cdot ph_{\text{}[i]}\\
\end{align}
$$

The code below fits a Gaussian regression model, with VOT (in milliseconds) as the outcome variable and phonation type as the (categorical) predictor. Phonation type (`phonation`) is coded with treatment contrasts.
