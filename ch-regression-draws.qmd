...
<!--# TODO: Use mald reaction times from chapter 23 -->
...
Fitting simple models with brms is relatively quick. However, more complex model using larger data sets can take some time (even hours!). It is useful to save the model fit to a file so that, once the model is fit once, you don't have to fit it again. Before you fit the model, create a folder called `cache/` folder in the RStudio project folder. We will use this folder to save the output of model fitting so that you don't have to refit the model every time. This is useful because as models get more and more complex, they can take quite a while to fit.
