...
For example, in a simple regression, like the one from the previous chapter, we want to know which combinations of intercept $\beta_0$, slope $\beta_1$ and standard deviation $\sigma$ values are most plausible given our prior beliefs and the observed data.
...
This type of probability is called the **joint posterior probability distribution** of the parameters.
In mathematical notation, the joint posterior probability distribution for a regression model is written as $P(\beta_0, \beta_1, \sigma \mid d)$, where $d$ is the observed data.
This is a Bayesian posterior distribution $P(h \mid d)$ like the one we saw in @sec-bayes (here $h = (\beta_0, \beta_1, \sigma)$, but the principle is the same).
The Greek letter $\theta$ (theta, \[ˈθiːtə\]) can be used to indicate any parameter (whether, an intercept, slope, mean, standard deviation, etc...) so we can generally define $h = (\theta_1, ..., \theta_k)$, where $k$ is the number of parameters.
In the regression model we are discussion, $k = 3$, so $(\theta_1, \theta_2, \theta_3)$ or using explicit notation $(\beta_0, \beta_1, \sigma)$.
...
\begin{aligned}
h & = (\theta_1, ..., \theta_k)\\
P(h \mid d) & \propto P(h) \cdot P(d|h)
\end{aligned}
$$

The **joint posterior probability distribution** is the posterior probability distribution of a vector of parameters $h$ conditional on observed data $d$.
:::

The joint posterior distribution can be thought of as a multidimensional landscape, with one dimension for each parameter.
Since we humans find it difficult to think in more than three dimensions, let's stick with a Gaussian model like the one from @sec-fit-model and let's assume data $d$ from $Gaussian(\mu = 10, \sigma = 2)$.
The model has two parameters: the mean $\mu$ and the standard deviation $\sigma$, so $h = (\mu, \sigma)$.
With these parameters, the joint posterior distribution is $P(\mu, \sigma \mid d)$ and can be thought of as a three-dimensional landscape, like the one in @fig-joint.
The *x*-axis represents possible values of the mean $\mu$, the *y*-axis possible values of the standard deviation $\sigma$, and the *z*-axis (the vertical axis) their posterior probability density given $d$.
This landscape (of the posterior probability density) is determined by the two components of Bayes' theorem: the prior probability distribution, $P(h)$, and the likelihood $P(d \mid h)$.
In this example, $h = (\mu, \sigma)$ denotes a particular combination of possible values for $\mu$ and $\sigma$.
So, the prior $P(h) = P(\mu, \sigma)$ is a the prior probability of the particular values $(\mu, \sigma)$ and the likelihood $P(d \mid \mu, \sigma)$ is the probability of the observed data $d$ given the particular values of $(\mu, \sigma)$.
Hills in the posterior probability landscape correspond to highly probable combinations of mean and standard deviation values given the priors and the data, whereas valleys correspond to combinations with low posterior density.

```{r}
#| label: fig-joint
#| fig-cap: "3D representation of the joint posterior probability distribution of $\\mu$ and $\\sigma$ from a Bayesian Gaussian model."
#| code-fold: true
...
posterior <- crossing(
  # h = (mu, sigma)
  mu = seq(8, 12, length.out = 100),
  sigma = seq(0.5, 3.5, length.out = 100)
) %>%
  mutate(
    # Likelihood P(d|h)
    log_likelihood = map2_dbl(
      mu, sigma,
      ~ sum(dnorm(data, mean = .x, sd = .y, log = TRUE))
    ),
    # Prior P(mu)
    log_prior_mu = dnorm(mu, mean = 10, sd = 5, log = TRUE),
    # Prior P(sigma)
    log_prior_sigma = dnorm(sigma, mean = 2, sd = 1, log = TRUE),
...
mu <- sort(unique(posterior$mu))
sigma <- sort(unique(posterior$sigma))
...
For realistic regression models, determining this joint posterior distribution analytically (i.e. by solving Bayes' theorem mathematically like we have done in the code of @fig-joint) is often very difficult or even impossible.
...
You first encountered MCMC in @sec-fit-model, when we ran a Gaussian model with `brm()`: the text printed when running `brm()` is about the MCMC algorithm.
...
In the three-dimensional landscape of a mean and a standard deviation (for a Gaussian model) like in @fig-joint, each draw therefore contains one mean value and one standard deviation value sampled together from the joint posterior distribution.
The algorithm repeats this process thousands of times, producing a large collection of posterior draws.
Regions of high posterior probability are sampled more often than regions of low posterior probability, so the collection of sampled values approximates the joint posterior distribution.
...
You can see a visual illustration of this process at <https://chi-feng.github.io/mcmc-demo/app.html>, a web app by Chi Feng.
In the top-right options box, select "standard" as the target distribution: this is a bivariate Gaussian distribution, which you can think of as the joint distribution of $\mu$ and $\sigma$ from above.
...
Looking only at the values across all draws for $\mu$ gives the posterior distribution of the mean, while looking only at the values across all draws for $\sigma$ gives the posterior distribution of the standard deviation.
These posterior distributions can be summarised as you would with any distribution: for example, you can take the mean and SD of the posterior draws of $\mu$ and $\sigma$.
...
The rest of this chapter shows how to extract and manipulate posterior draws from a fitted model by revisiting the model from @sec-regression.
...
It is useful to save the model fit to a file so that, once the model is fit once, you don't have to run the MCMC algorithm again.
...
You can see what that looks like in @fig-brms-model-object.
In the Viewer panel, click on the blue arrow next to `fit`, more objects will be revealed.
...
As mentioned above (and as we will see in more details below), the summary contains summary measures of the draws: `Estimate` is the mean of the posterior draws, `Est.Err` is their SD, and the CrIs are the CrIs of the posterior draws.
...
These summary measures are the mean (`Estimate`), the standard deviation (`Est.error`) of the draws and the lower and upper limits of the 95% Credible Interval (CrI) of the draws.
...
So a 90% CrI tells us that there is an 90% probability that the value is between the lower and upper limit, a 60% interval that there is a 60% probability and so on, conditional on the model and data.
...
With this model, `vow_bm`, getting all of these CrIs might sound very informative: the 95% CrIs are quite narrow, giving us quite a precise range of values for both the intercept and the coefficient of speech rate.
This is because there is quite a lot of data and the model is quite simple (there is only one predictor).
With more complex models and smaller data sets, uncertainty is greater and the intervals will span a larger range of values.
...
The median is another acceptable summary measure for posterior draws.
...
An alternative and quicker way to plot expected values is to use the `conditional_effects()` function from the brms package (you will learn how to easily calculate expected values in @sec-epred-draws).
...
The `b_speech_rate` draws tell us the difference for one unit increase in speech rate, so for example from 4 to 5 or from 6 to 7.
