...
Since we humans find it difficult to think in more than three dimensions, let's stick with a Gaussian model like the one from @sec-fit-model. This model had two parameters: the mean $\mu$ and the standard deviation $\sigma$.
...
You can see what looks like in @fig-brms-model-object. There, click on the blue arrow next to `fit`, more objects will be revealed.
...
## Plotting the expected value of vowel duration
...
A quick way to achieve this is to use the `conditional_effects()` function from the brms package.
...
The result is @fig-vow-bm-cond. The regression line has a negative slope: this means that with greater speech rate, vowel duration decreases.
