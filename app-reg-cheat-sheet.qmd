...
# Regression models cheat sheet  {#sec-reg-cheat-sheet}
...
The "simplest" regression model is the formula of a line:[^app-reg-cheat-sheet-1]

[^app-reg-cheat-sheet-1]: Technically, the "simplest" regression model is $y = f(x)$, but oh well...
...
The principles behind this formula can be extended to represent virtually any other type of regression model, independent of the nature of the outcome variable(s) ($y$), the predictor(s), the types of relationship between outcome and predictor, and so on.

**This means that if you master the principles of regression models, then you can virtually fit any kind of data using regression models.** You can bid farewell to classical ANOVAs, $t$-tests, $\chi^2$-tests, and what not.
...
Same goes with "logistic regression", "generalised regression models", "mixed-effects regression" and so on.
...
Ehm...
perhaps this is not going to win a poetry context, but...
...
-   The variable is *dichotomous*, i.e. it can take one of two levels: **Bernoulli** distribution.
    ...
    -   This family is fitted by default when you run `glm(..., family = binomial)`, aka "logistic regression" or "binomial regression" or with `brms::brm(..., family = bernoulli)`.[^app-reg-cheat-sheet-2]
...
[^app-reg-cheat-sheet-2]: Note that, despite using `family = binomial` in `glm()`, under the hood a Bernoulli distribution is used.
