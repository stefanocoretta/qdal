...
opts_2 <- c(
   ...
   answer = "The Phase Impenetrability Condition (PIC) states that only phase heads (C and v*) allow for extraction of elements from their complements so that movement out of non-phase domains (like TP or VP when not headed by a strong v*) should be impossible. However, *What was said about the budget?* shows exactly that type of movement. Auxiliary assumptions can be made to make it so that the PIC is not violated, like covert movement to phase edges.\n\n"
)

cat("b. Which of the following studies aims to improve the fit of a framework to the data?", longmcq(opts_2))
```
:::
