...
**Data transformation** is a fundamental aspect of data analysis. After the data you need to use is imported into R, you will often have to filter rows, create new columns, or join data frames, among many other transformation operations. In this chapter you will learn how to use `filter()` to filter the data and `mutate()` to mutate or create new columns.
...
Let's count the number of languages for each endangerment status using the new `Status` column. The dplyr package comes with a `count()` function and you can combine this with `group_by()`, as we did with `summarise()`, to get the number or rows based on the labels in one or more columns.
...
::: {.callout-important collapse="true"}
## Go Deeper: Assignment direction
