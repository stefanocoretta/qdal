# A workflow for quantitative studies {#sec-design}
...
We also recommend @gelman2020 and @schad2021 as complementing approaches.
...
In @sec-res-context, we introduced the research context framework proposed by @ellis2008. The framework is an aid to defining the object of study, from a more general perspective (the topic) down to a specific research question and hypothesis.
...
Most quantitative research, while not mecessarily framed in causal terms, is ultimately about causality [@mcelreath2020].
...
Once the resercher is satisfied with the prior predictive checks, then they can use the chosen priors in the chosen regression model to check that the model works as expected.
...
Estimating the sample size necessary to reach the desired precision in the estimates of interests (these are determined by the DAG) is not necessarily a separate step than the data simulation and model checks from the previous section, but it is worth mentioning it separately.
...
The following steps will imply either of these events have taken place and that you completed data collection.
