...
where $\epsilon$ is the error. In other words, $y$ is the sum of $\beta_0$, $\beta_1 * x$ and some error. In regression modelling, the error $\epsilon$ is assumed to come from a Gaussian distribution with mean 0 and standard deviation $\sigma$: $Gaussian(\mu = 0, \sigma)$.
...
Now, the goal of a regression model is to estimate $\beta_0$, $\beta_1$ and $\sigma$ from the data (i.e. from the values of $x$ and $y$). In the following chapters, you will learn how to fit regression models to a variety of data to answer research questions.
