...
-   `genderM` is $\beta_1$: this is the difference in mean f0 between male and female gender, *when attitude is informal* (i.e. at the reference level of attitude).
...
-   `attitudepol` is $\beta_2$: this is the difference in mean f0 between polite and informal attitude, *when gender is female* (i.e. at the reference level of gender).
...
## Model's draws and expected predictions
...
To calculate the expected predictions of f0, we plug in the columns from the draws object `f0_bm_int_draws` according to the model's formula.
...
Let's plot the distributions of the expected predictions we just calculated.
We need to pivot the draws object: to make plotting easier, we need a column with the gender value and one column with the attitude value, and also a column with the expected prediction values.
...
The two new values are input into the specified new columns `gender` and `attitude` (the user picks and specifies the names as one of the arguments).
...
Now we can plot the expected predictions with ggplot2 (@fig-f0-bm-int-epred).
...
#| fig-cap: "Density plot of the expected predictions of f0 by gender and attitude."
...
By default, it plots the 50-80-95% CrIs of the expected predictions.

```{r}
#| label: fig-f0-bm-int-epred-interval
#| fig-cap: "Intervals plot of the expected predictions of f0 by gender and attitude."
...
For example, the 95% CrI of the $\beta_3$ coefficient suggests a good amount of uncertainty in how the attitude effect differs between female and male participants.
Furthermore, we can compute summary measures of the attitude difference in male participants directly.
...
The mean difference is -11 Hz (SD = 8) and the 95% CrI is \[-27, +5\] Hz.
This is somewhat indicative of a difference (polite speech is produced with a somewhat lower f0) but it is also possible that there is instead a negligible positive difference (+5 Hz).
