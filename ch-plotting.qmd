...
In the previous chapter, @sec-viz, you have learnt about basic visualisation principles.
...
With these principles in mind, this chapter will teach you the basics of data visualisation (aka plotting) in R. In R, you can create plots using different systems: base R, [ggplot2](https://ggplot2.tidyverse.org/index.html), [plotly](https://plotly-r.com), [lattice](https://lattice.r-forge.r-project.org) and others. This book focusses on the ggplot2 system, which is part of the tidyverse (you will learn the basics of plotly as well to create interactive plots, in a later chapter), but before we dive in, it is useful to have a look at the base R plotting system.
...
We can read the data with `read_csv()` from readr and plot it with `ggplot()` from ggplot2. Since both readr and the ggplot2 package are part of the tidyverse, it is sufficient to attach the tidyverse with `library(tidyverse)`.
...
The `polite` data contains several acoustic measurements from utterances spoken by Korean students in Germany. Each row is a single utterance and each participant has spoken many utterances. These are the columns that we need to produce @fig-final-plot.
...
You can specify the data and mapping with the `data` and `mapping` arguments of the `ggplot()` function. Note that the `mapping` argument is always specified with `aes()`: `mapping = aes(…)`. In the following bare plot, we are just mapping `f0mn` to the *x*-axis and `H1H2` to the *y*-axis, from the `polite` data frame. From this point on I will assume you'll be creating a new code chunk (XXX make sure you introduced Quarto), copy-paste the code and run it, without explicit instructions.
...
::: callout-important
#### Extra: The Layered Grammar of Graphics
...
Wait, what is that thing, `|>`? It's called a **pipe**. Think of a pipe as a teleporter. The pipe `|>` teleports the data `polite` into the following function as the first argument. So `polite |> ggplot()` is equivalent to `ggplot(polite)`. For now it might not make much sense using the pipe, but later in the book you will start to chain many functions one after the other using the pipe, at which point its usefulness will be more obvious. For example, you will filter the data before plotting it, like so:
...
As briefly mentioned earlier, there seems to be two subgroups within the data: one below about 175 Hz and one above it. These subgroups are in fact related to the `gender` of the participants. We can colour the points by gender, using the `colour` aesthetic.[^ch-plotting-2]
...
The default colour palette is used, but you can customise it. One way to quickly change the palette it to use one of the `scale_colour_*()` functions. The relevant function for our plot is `scale_colour_brewer()`. This function creates palettes based on [ColorBrewer 2.0](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3). There are three types of palettes (see the linked website for examples):
