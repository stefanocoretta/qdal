...
![](img/data-viz.png){fig-align="center" width="500"}

As you learned in [Chapter 2](#0), quantitative data analysis can be concieved as three activities: summarising, visualising and modeling data. This chapter introduces you to basic principles of good data visualisation, while in @sec-plotting you will learn the basics of plotting data in R.
...
Let's see a few examples that illustrate each point.
...
Let's use the `glot_status` data. You will not see the code used to create the plots because you will learn about it later.
...
The `albvot` data contains data on VOT in Albanian. It has data from 6 speakers [@coretta2022].
...
The plot below shows the number of gestures enacted by infants of English, Bengali and Chinese background as recorded during a controlled session [@cameronfaulkner2020].
...
Three different types of gestures are shown: hold out and give gestures (`ho_gv`), index-finger pointing (`point`) and reach out gestures (`reach`). Moreover the plot shows the number of gestures at 10 and 12 months.
